...
let rec expr_to_term (expr:expression) : term =
  ...
  | Pexp_apply ({pexp_desc = Pexp_ident {txt=Lident i; _}; _}, [(_, a); (_, b)]) ->
      begin match i with
      | "^" -> BinOp (SConcat, expr_to_term a, expr_to_term b)
      | "+" -> BinOp (Plus, expr_to_term a, expr_to_term b)
      | "-" -> BinOp (Minus, expr_to_term a, expr_to_term b)
...
let rec transformation (bound_names:string list) (expr:expression) : core_lang =
  ...
  | Pexp_constant {pconst_desc = c; _} ->
    begin match c with
    | Pconst_integer (i, _) -> CValue (Const (Num (int_of_string i)))
    | Pconst_string (s, _, _) -> CValue (Const (TStr s))
    | _ -> failwith (Format.asprintf "unknown kind of constant: %a" Pprintast.expression expr)
    end
  (* lambda *)
  | Pexp_function (_, _, _body) ->
